...
open(OUT1,">","$dir/08-02Intron-retation.bed");
foreach(@IN3){
   if(/SN:(chr_\d+)\s+LN:(\d+)\s/){ $chr=$1; $chr_len=$2; undef%AS;undef%AS_other;
      ...
      foreach my$line(@IN1){if($line=~/^$chr\s+(\d+)\s+(\d+)\s/){ my$start=$1; my$end=$2; chomp$line; my$MARK=0; my$a=0; my$b=0;
            if(!exists$AS{$1} and !exists$AS_other{$1}){ $MARK++; } 
            foreach my$keys(keys%AS){if($end==$AS{$keys}){$a=1;} } if($a==0){$MARK++; } 
            foreach my$keys(keys%AS_other){if($end==$AS_other{$keys}){$b=1;} } if($b==0){$MARK++; }
            if($MARK==2){  push@IR,$line; 
