use strict;
my($n,%chr, @chr, $chr, @gene_corrd, %gene, $contig_end, $gene_id, $gene_start, $gene_end, $former, $next, %new_nano_gene, $min);
my$dir=$ENV{'PWD'};
open(IN1,"<$ARGV[0]"); #7_GFF_sorted
...
my@GFF=<IN1>; close IN1; foreach(@GFF){ if(/(chr_\d+)\s+.+?\s+(\d+)\s+(\d+)\s.+?/){ $chr{$1}=0;  }  } 
#my@temp1=<IN3>; my@temp2=<IN4>;
my@total_GTF=<IN5>; 
foreach $chr(keys%chr){
...
open(OUT1,">$dir/8conju_gap_new_gene.gtf");
